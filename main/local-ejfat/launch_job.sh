...
helm install "$ID-job-$INDEX" job/ \
  ...
  --set Job.batch.spec.template.spec.containers[0].ports[0].containerPort="$ERSAP_EXPORTER_PORT" \
  --set Job.batch.spec.template.spec.containers[0].ports[1].containerPort="$PROCESS_EXPORTER_PORT" \
  --set Job.batch.spec.template.spec.containers[0].ports[2].containerPort="$EJFAT_EXPORTER_PORT" \
  --set Job.batch.spec.template.spec.containers[1].ports[0].containerPort="$JRM_EXPORTER_PORT" \
  --set Job.batch.spec.template.spec.containers[1].ports[1].containerPort="$ERSAP_QUEUE_PORT"